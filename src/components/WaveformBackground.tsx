...
const WaveformBackground = () => {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    tiles.forEach((tile, index) => {
      ...
      // Interpolate color from dark gray to electric blue
      const r = Math.round(42 + (59 - 42) * glowIntensity);
      const g = Math.round(42 + (130 - 42) * glowIntensity);
      const b = Math.round(42 + (246 - 42) * glowIntensity);
      ...
      if (glowIntensity > 0.1) {
        ctx.shadowColor = `rgba(59, 130, 246, ${glowIntensity * 0.8})`;
